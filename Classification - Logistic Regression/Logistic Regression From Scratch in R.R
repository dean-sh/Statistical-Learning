...
update.Theta = function(theta, d, y, x, n, alpha){
  ...
  # a = t(replicate(d,y)) * x
  a = replicate(d,y, simplify = TRUE) * x
  
  # theta.x = t(replicate(n, theta)) * t(x)
  theta.x = replicate(n, theta) * t(x)
...
logistic = function(x, theta){
  model = sapply(1:ncol(X) , function(index) 1 / (1 + exp(sum(theta * X[,index]))))
  return(model)
}

p = ncol(df)-1
theta = rep(0.1, p) #theta = vector length 1xP
...
X = model.matrix(binaryMpg~., data = df)[,-1] #We want just the feaures, without the bias term
#x ~ [n,p]
n = nrow(X)
...
finaltheta = Theta.Optimization(theta, X, y, threshold, alpha, max_iter)

predictions = logistic(X, finaltheta)
